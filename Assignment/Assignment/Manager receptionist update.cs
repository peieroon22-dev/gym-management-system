...
{
    public partial class Manager_update : BaseForm
    {
        ...
        {
            string receptionistID = txt_ID.Text.Trim();
            ...
            {
                MessageBox.Show("Receptionist ID is required.", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    {
                        new SqlParameter("@ReceptionistID", receptionistID)
                    };
                    ...
                    if (isFullTime || isPartTime)
                    {
                        ...
                        {
                            updates.Add("Employment_Status = @EmploymentStatus");
                    ...
                    if (updates.Count == 0)
                    {
                        MessageBox.Show("No fields to update. Please provide at least one field other than Receptionist ID.", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }

                    string query = $"UPDATE [dbo].[Receptionist] SET {string.Join(", ", updates)} WHERE Receptionist_ID = @ReceptionistID";
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            MessageBox.Show("Receptionist details updated successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            btn_clear.PerformClick();
                        }
                        else
                        {
                            MessageBox.Show("No receptionist found with the given ID.", "Update Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show("An error occurred while updating the receptionist details: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void btn_clear_Click(object sender, EventArgs e)
        {
            txt_ID.Text = txt_name.Text = txt_password.Text = txt_phoneNumber.Text = txt_email.Text = txt_home.Text =  "";
